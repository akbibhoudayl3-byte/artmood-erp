...
import { generateBOM, computeBOMCost } from '@/lib/services/kitchen-bom-engine';
...
/** POST /api/kitchen/bom — Generate BOM for a kitchen project */
export async function POST(request: NextRequest) {
  ...
  const [kitchenRes, modulesRes, fillersRes, productModulesRes] = await Promise.all([
    ...
    supabase.from('product_modules').select(`
      *,
      module_rules(*),
  ...
  const costs = computeBOMCost(bom, kitchen);

  return NextResponse.json({ bom, costs });
}
