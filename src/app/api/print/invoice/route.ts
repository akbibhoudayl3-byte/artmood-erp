...
export async function GET(request: NextRequest) {
  ...
  const [projRes, quotesRes, paymentsRes] = await Promise.all([
    ...
    supabase.from('quotes').select('*, quote_lines(*)').eq('project_id', projectId).eq('status', 'accepted').order('version', { ascending: false }).limit(1),
  ...
  await writeAuditLog({
    ...
    entity_type: 'project',
    entity_id: projectId,
    notes: 'invoice printed',
  });

  const project = projRes.data;
  const quote = quotesRes.data?.[0];
  const payments = paymentsRes.data || [];
  const totalPaid = payments.reduce((s: number, p: any) => s + Number(p.amount), 0);
  const remaining = Number(project.total_amount) - totalPaid;

  const quoteLines = (quote?.quote_lines || []).sort((a: any, b: any) => a.sort_order - b.sort_order);

  const today = new Date();
  const invoiceNumber = `FAC-${today.getFullYear()}-${esc(project.reference_code)}`;

  const lineRows = quoteLines.map((l: any, i: number) => `
    <tr>
  ...
  const subtotal = quote?.subtotal ?? project.total_amount;
  const discountPercent = quote?.discount_percent ?? 0;
  const discountAmount = quote?.discount_amount ?? 0;
  const totalTTC = quote?.total_amount ?? project.total_amount;
...
<body>
  <div class="header">
    ...
    <div style="text-align:right;">
      ...
      <p style="font-size:12px;color:#64648B;margin-top:8px;">
        Date : ${today.toLocaleDateString('fr-FR', { year: 'numeric', month: 'long', day: 'numeric' })}
      </p>
    </div>
  </div>
  ...
  <p style="padding:12px;color:#64648B;">Aucun devis accepté associé. Montant total du projet : <strong>${Number(project.total_amount).toLocaleString('fr-MA')} MAD</strong></p>
  `}

  <div class="totals">
    <table class="totals-table">
      <tr><td>Sous-total HT</td><td>${Number(subtotal).toLocaleString('fr-MA')} MAD</td></tr>
      ${discountAmount > 0 ? `<tr><td>Remise (${discountPercent}%)</td><td style="color:#dc2626;">-${Number(discountAmount).toLocaleString('fr-MA')} MAD</td></tr>` : ''}
      <tr class="grand-total"><td>TOTAL TTC</td><td>${Number(totalTTC).toLocaleString('fr-MA')} MAD</td></tr>
      <tr><td style="padding-top:12px;">Total payé</td><td style="padding-top:12px;color:#16a34a;">+${totalPaid.toLocaleString('fr-MA')} MAD</td></tr>
  ...
  <div class="footer">
    ...
    <p>Document généré automatiquement le ${today.toISOString()} — Ne constitue pas une facture fiscale officielle</p>
  </div>
</body>
</html>`;
