...
export async function POST(
  ...
  const { data: invoice, error: invErr } = await supabase
    .from('invoices')
    .select('id, invoice_number, project_id, total_amount, total_ttc, vat_rate, vat_amount, paid_amount, status')
    .eq('id', invoiceId)
    .single();
  ...
  // 2. Block overpayment (compare against total_ttc which includes VAT)
  ...
  await writeAuditLog({
    ...
    notes: `Payment ${amount} MAD on invoice ${invoice.invoice_number}`,
  });
  ...
  return NextResponse.json({
    ...
    invoice: {
      ...
      remaining: invoiceTotal - newPaidAmount,
      status: newStatus,
    },
    project: {
      project_id: invoice.project_id,
