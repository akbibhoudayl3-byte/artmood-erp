...
import { CheckCircle, XCircle, Clock, ArrowLeft, X } from 'lucide-react';
...
interface ExReq {
  ...
  created_at: string;
  project: { reference_code: string; client_name: string; total_amount: number; paid_amount: number };
...
export default function ExceptionRequestsPage() {
  ...
  async function load() {
    ...
    let q = supabase
      ...
      .select('*, project:projects(reference_code, client_name, total_amount, paid_amount), requester:profiles!project_exceptions_requested_by_fkey(full_name), reviewer:profiles!project_exceptions_reviewed_by_fkey(full_name)')
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {requests.map(req => {
              const pct = req.project.total_amount > 0 ? Math.round((req.project.paid_amount / req.project.total_amount) * 100) : 0;
              return (
                <Card key={req.id}>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 space-y-1.5">
                        ...
                        <p className="text-xs text-gray-500">
                          Acompte: <span className="font-semibold text-red-600">{pct}%</span>
                          {' '}({req.project.paid_amount?.toLocaleString()} / {req.project.total_amount?.toLocaleString()} MAD)
                        </p>
