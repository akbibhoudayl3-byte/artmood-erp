            ...
            {canWrite && (
              <div className="space-y-2">
                <Button
                  ...
                  size="sm"
                  className="w-full bg-[#1E2F52] hover:bg-[#2a3f6b]"
                  disabled={generatingBom}
                >
                  {generatingBom ? <RefreshCw size={14} className="animate-spin" /> : <Zap size={14} />}
                  {generatingBom ? 'Génération BOM…' : 'Générer Ordre depuis BOM'}
                </Button>
                <Button onClick={() => setShowNewOrder(true)} size="sm" variant="secondary" className="w-full">
                  <Plus size={14} /> Nouvel Ordre Manuel
                </Button>
              </div>
            )}
